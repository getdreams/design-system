...
Then all tokens definitions are merged in the `~/tokens/theme/` component to build the global Tailwind CSS `theme.extend` definition. This `theme` is used in the `tailwind.config.js` file directly to setup all the classes and variants.
